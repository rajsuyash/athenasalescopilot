...
const HEADING_PATTERNS: Array<{ regex: RegExp; stage: string }> = [
  ...
  {
    regex: /^#{1,4}\s*(eagle.?s?\s*eye|pillar|delivery\s*model|temperature\s*check)\b/i,
    stage: 'demo',
  },
  { regex: /^#{1,4}\s*(c6|c\s*6|consequence)\b/i, stage: 'objection_handling' },
  {
    regex: /^#{1,4}\s*(c7|c\s*7|goal|commitment|price\s*reveal|closing|close|next\s*step)\b/i,
    stage: 'closing',
  },
];
...
/** Stable name for the one auto-generated, BMC-derived collection per
 *  workspace. We republish into this single collection (new version, repointed
 *  currentVersionId) on every BMC change rather than creating a fresh
 *  timestamped collection each time — otherwise N edits would leave N
 *  collections all serving stale blocks (loadStageBlocksAsChunks merges every
 *  collection whose currentVersionId is set). */
const AUTO_COLLECTION_NAME = 'BMC auto-generated playbook';
...
export async function generateScriptFromBmc(
  ...
  if (parsed.length < 3) {
    throw Object.assign(
      new Error(
        `generator output had too few stage blocks (got ${parsed.length}); model may have failed to follow the format`,
      ),
      { statusCode: 502, code: 'PARSE_TOO_FEW_BLOCKS' },
    );
  }

  // Mid-flight staleness guard: generation takes tens of seconds, and every
  // BMC edit fires its own runPlaybookGen. If the BMC was bumped again while
  // we were generating, abort — the newer run produces the authoritative
  // script, and we must not clobber it with an older one.
  const fresh = await getBmc(args.workspaceId);
  if (fresh && fresh.version !== bmc.version) {
    throw Object.assign(
      new Error(
        `bmc changed mid-generation (was v${bmc.version}, now v${fresh.version}) — aborting stale script gen`,
      ),
      { statusCode: 409, code: 'BMC_VERSION_CHANGED' },
    );
  }

  // Tx: upsert the singleton auto-collection, append a new published version
  // holding all parsed blocks, atomically repoint currentVersionId, and stamp
  // sourceBmcVersion for idempotency short-circuiting by runPlaybookGen.
  const result = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
    const existing = await tx.scriptCollection.findFirst({
      where: { workspaceId: args.workspaceId, isAutoGenerated: true },
      include: { versions: { orderBy: { version: 'desc' }, take: 1 } },
    });
    const collection =
      existing ??
      (await tx.scriptCollection.create({
        data: {
          workspaceId: args.workspaceId,
    ...
    const nextVersion = existing?.versions[0] ? existing.versions[0].version + 1 : 1;
    ...
    await tx.scriptCollection.update({
      ...
      data: { currentVersionId: version.id, sourceBmcVersion: bmc.version },
    });
